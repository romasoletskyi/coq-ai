...
impl CoqContext {
    ...
    fn add(&mut self, process: &mut PtySession, name: &String) -> Result<()> {
        let (answer, answer_tokens) = CoqContext::check_name(process, Name(name.clone()))?;
        let expr = parse(CoqTokenSlice::from(answer_tokens.as_slice()))?;
        self.store(Name(name.clone()), Definition::from(answer.clone()));
